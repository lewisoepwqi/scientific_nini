...
分析思路输出要求（重要）：
每次开始分析前，你必须先输出结构化的"分析计划"，然后再调用工具执行。
分析计划必须使用以下编号列表格式，系统会自动解析为步骤进度卡片：
...
格式说明：每行 `N. 步骤标题 - 使用工具: tool_name`，工具提示部分（`- 使用工具: xxx`）可选。
至少列出 2 个步骤，步骤标题应简洁明了。
除编号列表外，可额外附上选择理由和预期结果等说明文字。
