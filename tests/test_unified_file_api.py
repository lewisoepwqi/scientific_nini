...
def create_plotly_json(session_id: str, filename: str) -> Path:
    ...
    chart_data = {
        "data": [{"x": [1, 2, 3], "y": [4, 5, 6], "type": "scatter", "mode": "lines+markers"}],
...
class TestUnifiedFileDownload:
    ...
    def test_download_file_with_subpath(self, client: TestClient, session_id: str):
        ...
        response = client.get(
            f"/api/workspace/{session_id}/files/nested/folder/file.txt?download=1"
        )
    ...
    def test_download_with_inline_parameter(self, client: TestClient, session_id: str):
        ...
        # inline=true
        response = client.get(
            f"/api/workspace/{session_id}/files/inline_test.txt?download=1&inline=1"
        )
        assert response.status_code == 200
...
class TestPlotlyJsonConversion:
    ...
    def test_plotly_json_raw_parameter(self, client: TestClient, session_id: str):
        ...
        response = client.get(
            f"/api/workspace/{session_id}/files/chart.plotly.json?download=1&raw=1"
        )
...
"""
        ...
        # 请求 bundle 下载
        response = client.get(
            f"/api/workspace/{session_id}/files/notes/new_format_report.md?bundle=1"
        )
        ...
        with zipfile.ZipFile(zip_buffer, "r") as zf:
            ...
            # 新格式的 URL 应该被替换
            assert (
                f"/api/workspace/{session_id}/files/artifacts/correlation_chart.png"
                not in md_content_in_zip
            )
            # 应该使用相对路径
...
class TestEditFileCompatibility:
    ...
    def test_edit_file_in_root_downloadable(self, client: TestClient, session_id: str):
        ...
        create_test_file(
            session_id, "analysis_report.md", "# Analysis Report\n\nGenerated by edit_file."
        )
    ...
    def test_edit_file_with_folder_downloadable(self, client: TestClient, session_id: str):
        ...
        create_test_file(session_id, "analysis/chapter1.md", "# Chapter 1\n\nContent here.")
...
class TestImageFileDirectAccess:
    ...
    def test_image_file_direct_stream_without_download_param(
        self, client: TestClient, session_id: str, ext: str
    ):
...
"""
        ...
        img_response = client.get(
            f"/api/workspace/{session_id}/files/artifacts/correlation_heatmap.png"
        )
        ...
        assert (
            f"/api/workspace/{session_id}/files/artifacts/correlation_heatmap.png"
            in data["data"]["content"]
        )

    @pytest.mark.parametrize("ext", ["pdf", "docx", "xlsx", "zip"])
    def test_binary_file_direct_stream_without_download_param(
        self, client: TestClient, session_id: str, ext: str
    ):
    ...
    def test_non_image_file_still_returns_json_by_default(
    ...
    def test_unicode_filename_content_disposition(self, client: TestClient, session_id: str):
        ...
        # 下载文件
        response = client.get(
            f"/api/workspace/{session_id}/files/artifacts/{chinese_filename}?download=1"
        )
        assert response.status_code == 200
    ...
    def test_unicode_filename_download_headers_various_chars(
        self, client: TestClient, session_id: str
    ):
    ...
    def test_pdf_inline_parameter_respected(self, client: TestClient, session_id: str):
        ...
        response_inline = client.get(
            f"/api/workspace/{session_id}/files/artifacts/{pdf_filename}?inline=1"
        )
        ...
        response_download = client.get(
            f"/api/workspace/{session_id}/files/artifacts/{pdf_filename}?download=1"
        )
