...
- [ ] 8.1 端到端验证：首次启动 → 发消息 → 试用激活 → 正常响应（需运行时验证）
- [ ] 8.2 端到端验证：配置 DeepSeek 密钥 → 拉取模型列表 → 选模型 → 保存 → 对话正常（需运行时验证）
- [ ] 8.3 端到端验证：切换供应商（DeepSeek → 智谱）→ 旧供应商停用 → 新供应商生效（需运行时验证）
- [ ] 8.4 端到端验证：模拟试用到期 → 发消息 → 显示阻断提示（需运行时验证）
