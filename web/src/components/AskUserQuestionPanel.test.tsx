...
describe("AskUserQuestionPanel", () => {
 ...
 expect(screen.getByRole("tab", { name: /数据范围.*当前/u })).toHaveAttribute(
 "aria-current",
 "step",
 );
 expect(screen.getByText("当前处理：数据范围")).toBeInTheDocument();
 expect(
 within(screen.getByRole("tabpanel")).getByText("你希望分析哪个数据范围？"),
 ).toBeInTheDocument();
 expect(
 within(screen.getByRole("tabpanel")).queryByText("结果需要什么输出格式？"),
 ).not.toBeInTheDocument();

 fireEvent.click(screen.getByRole("tab", { name: /输出格式/u }));

 expect(screen.getByRole("tab", { name: /输出格式.*当前/u })).toHaveAttribute(
 "aria-current",
 "step",
 );
 expect(screen.getByText("当前处理：输出格式")).toBeInTheDocument();
 expect(
 within(screen.getByRole("tabpanel")).getByText("结果需要什么输出格式？"),
 ).toBeInTheDocument();
 expect(
 within(screen.getByRole("tabpanel")).queryByText("你希望分析哪个数据范围？"),
 ).not.toBeInTheDocument();
 });
 ...
 expect(screen.getByRole("tab", { name: /数据范围.*已跳过/u })).toBeInTheDocument();
