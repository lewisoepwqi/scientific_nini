    ...
    <div className="mt-2 space-y-1.5">
      {artifacts.map((artifact, idx) => {
        ...
        <a
          ...
          href={href}
          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700
                     bg-white dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 text-sm text-slate-700 dark:text-slate-300
                     transition-colors cursor-pointer mr-2"
        >
          {getIcon(artifact)}
          <span className="truncate max-w-[200px]">{artifact.name}</span>
          <Download size={12} className="text-slate-400 dark:text-slate-500 flex-shrink-0" />
        </a>
        )
      })}
    </div>
