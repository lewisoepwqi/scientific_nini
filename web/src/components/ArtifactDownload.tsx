...
export default function ArtifactDownload({ artifacts }: Props) {
 ...
 href={href}
 className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[var(--border-default)]
 bg-[var(--bg-base)] hover:bg-[var(--accent-subtle)] hover:border-[var(--accent)] text-sm text-[var(--text-secondary)]
 transition-colors cursor-pointer mr-2"
 >
 {getIcon(artifact)}
 <span className="truncate max-w-[200px]">{artifact.name}</span>
 <Download size={12} className="text-[var(--text-muted)] flex-shrink-0" />
 </a>
 )
 })}
 </div>
