  ...
  return (
    <>
      ...
      <div className="fixed inset-4 z-50 flex items-center justify-center pointer-events-none">
        <div
          className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-3xl max-h-full flex flex-col pointer-events-auto"
          onClick={(e) => e.stopPropagation()}
        >
          {/* 头部 */}
          <div className="flex items-center justify-between px-4 py-3 border-b dark:border-slate-700 flex-shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300 truncate">
...
function PreviewContent({ preview }: { preview: PreviewData }) {
  switch (preview.preview_type) {
    ...
    case 'text':
      ...
      return (
        <div>
          <pre className="text-xs font-mono bg-gray-50 dark:bg-slate-800 rounded-lg p-4 overflow-x-auto whitespace-pre-wrap break-words">
            {preview.content}
          </pre>
          {preview.total_lines && preview.preview_lines && preview.total_lines > preview.preview_lines && (
            <div className="mt-2 text-[10px] text-gray-400 dark:text-slate-500 text-center">
    ...
    case 'pdf':
      ...
      return (
        <div className="text-center py-12 text-sm text-gray-500 dark:text-slate-400">PDF 预览地址不可用</div>
      )
