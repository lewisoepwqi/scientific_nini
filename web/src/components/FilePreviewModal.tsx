...
export default function FilePreviewModal() {
 ...
 {error && (
 <div className="flex items-center justify-center h-64 text-[var(--error)] text-sm">
 {error}
 </div>
 )}
...
function PreviewContent({ preview }: { preview: PreviewData }) {
 ...
 return (
 <div>
 <pre className="text-xs font-mono bg-[var(--bg-elevated)] rounded-lg p-4 overflow-x-auto whitespace-pre-wrap break-words">
 {preview.content}
 </pre>
 {preview.total_lines && preview.preview_lines && preview.total_lines > preview.preview_lines && (
 <div className="mt-2 text-[10px] text-[var(--text-muted)] text-center">
 ...
 return (
 <div className="text-center py-12 text-sm text-[var(--text-secondary)]">PDF 预览地址不可用</div>
 )
