...
def _default_env_content() -> str:
    return (
        ...
        "\n"
        "# 可选：系统内置模型（阿里百炼）\n"
        "NINI_BUILTIN_DASHSCOPE_API_KEY=\n"
        "NINI_BUILTIN_DASHSCOPE_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1\n"
        "NINI_BUILTIN_CHAT_FAST_MODEL=qwen3.5-27b\n"
        "NINI_BUILTIN_CHAT_DEEP_MODEL=qwen3.5-plus\n"
        "NINI_BUILTIN_IMAGE_FAST_MODEL=qwen3-vl-8b-instruct\n"
        "NINI_BUILTIN_IMAGE_DEEP_MODEL=qwen-vl-plus\n"
        "NINI_BUILTIN_TITLE_MODEL=qwen2.5-14b-instruct\n"
        "\n"
        "# 可选：MiniMax\n"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    def _register_tool_subcommands(
        ...
        *,
        ...
        sub = parent.add_subparsers(dest=dest_name, required=True)

        list_cmd = sub.add_parser("list", help="列出所有已注册能力")
        ...
        create_cmd.set_defaults(func=_cmd_skills_create)

        export_cmd = sub.add_parser("export", help="导出能力为指定格式")
    ...
    tools_parser = subparsers.add_parser(
        "tools", help="管理工具（Function Tools + Markdown Skills）"
    )
    _register_tool_subcommands(tools_parser, dest_name="tools_command")
