...
class VisualizationCapability:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # 合并所有图表
        result.charts = (
            result.distribution_charts
            + result.relationship_charts
            + result.composition_charts
        )

        # Step 5: 导出图表
        if export_format and result.charts:
            result.exported_files = await self._export_charts(
                session, result.charts, export_format
            )
