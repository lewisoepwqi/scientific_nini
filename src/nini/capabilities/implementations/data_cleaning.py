...
class DataCleaningCapability:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if strategy == "auto":
            ...
            operations = await self._aggressive_clean(
                session, df, dataset_name, outlier_method
            )
        else:
    ...
    def _generate_recommendations(self, result: DataCleaningResult) -> list[str]:
        ...
        # 数据损失评估
        loss_pct = (result.rows_removed / result.original_rows * 100) if result.original_rows > 0 else 0
        if loss_pct > 20:
            recommendations.append(f"数据损失率较高 ({loss_pct:.1f}%)，建议检查清洗策略是否过于激进")
        elif loss_pct > 5:
    ...
    def _generate_interpretation(self, result: DataCleaningResult) -> str:
        ...
        if result.operations:
            ...
            for op in result.operations:
                lines.append(f"- **{op.operation_type}** | {op.column}: {op.method} ({op.rows_affected} 行)")
            lines.append("")
