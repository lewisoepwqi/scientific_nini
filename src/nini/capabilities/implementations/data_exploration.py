...
class DataExplorationCapability:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        result.success = True
        result.message = f"成功探索数据集 '{dataset_name}'，发现 {len(result.column_profiles)} 个变量"
    ...
    def _generate_recommendations(self, result: DataExplorationResult) -> list[str]:
        ...
        if result.missing_pct > 20:
            recommendations.append(f"数据集缺失值比例较高 ({result.missing_pct}%)，建议检查数据收集流程")
        elif result.missing_pct > 0:
            recommendations.append(f"存在 {result.total_missing} 个缺失值，建议进行缺失值处理")

        # 重复行建议
        if result.duplicate_pct > 5:
            recommendations.append(f"存在 {result.duplicate_rows} 行重复数据 ({result.duplicate_pct}%)，建议去重")

        # 异常值建议
        if result.outlier_summary:
            cols_with_outliers = list(result.outlier_summary.keys())
            recommendations.append(f"以下列存在异常值: {', '.join(cols_with_outliers)}，建议检查或处理")
    ...
    def _generate_interpretation(self, result: DataExplorationResult) -> str:
        ...
        if result.outlier_summary:
            lines.extend([
                "### 异常值检测",
                *[f"- {col}: {count} 个异常值" for col, count in result.outlier_summary.items()],
                "",
            ])
        ...
        if result.high_correlations:
            lines.extend([
                "### 高相关性变量 (|r| > 0.8)",
                *[f"- {c['var1']} vs {c['var2']}: r = {c['correlation']}" for c in result.high_correlations[:5]],
                "",
            ])
