...
class RegressionAnalysisCapability:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if check_multicollinearity and len(independent_vars) > 1:
            ...
            result.high_vif_vars = [
                var for var, vif in vif_results.items() if vif > vif_threshold
            ]
        ...
        for var in independent_vars:
            if var in coeffs:
                ...
                result.coefficients.append(
                    self._extract_coefficient(var, coef_data)
                )
    ...
    async def _create_visualizations(
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("success") and result.get("artifacts"):
                ...
                artifacts.extend([a.to_dict() if hasattr(a, "to_dict") else a for a in result.artifacts])
        except Exception:
            pass
    ...
    def _record_enriched_result(
        ...
        if result.f_pvalue is not None:
            sig = "***" if result.f_pvalue < 0.001 else "**" if result.f_pvalue < 0.01 else "*" if result.f_pvalue < 0.05 else ""
        ...
        for coef in result.coefficients:
            if coef.significant:
                _record_stat_result(
                    ...
                    message=f"β = {coef.coefficient:.4f}, p = {coef.p_value:.4f}" if coef.p_value else f"β = {coef.coefficient:.4f}",
    ...
    def _generate_model_summary(self, result: RegressionAnalysisResult) -> str:
        ...
        if result.f_statistic is not None and result.f_pvalue is not None:
            sig = "***" if result.f_pvalue < 0.001 else "**" if result.f_pvalue < 0.01 else "*" if result.f_pvalue < 0.05 else "ns"
    ...
    def _generate_interpretation(self, result: RegressionAnalysisResult) -> str:
        ...
        parts.append("\n## 模型拟合度")
        if result.r_squared is not None:
            parts.append(f"R² = {result.r_squared:.3f}（模型解释了因变量 {result.r_squared*100:.1f}% 的变异）")
        if result.adj_r_squared is not None:
            parts.append(f"调整 R² = {result.adj_r_squared:.3f}")

        if result.f_pvalue is not None:
            sig_text = "显著" if result.f_pvalue < 0.05 else "不显著"
            parts.append(f"F 检验: F = {result.f_statistic:.2f}, p = {result.f_pvalue:.4f}（模型整体 {sig_text}）")
        ...
        if nonsig_coefs:
            ...
            for coef in nonsig_coefs:
                parts.append(f"- {coef.variable}: β = {coef.coefficient:.4f}, p = {coef.p_value:.4f}")
        ...
        if result.diagnostics:
            ...
            if result.diagnostics.residual_normality_p is not None:
                ...
                parts.append(f"残差正态性: {status} (p = {result.diagnostics.residual_normality_p:.3f})")
