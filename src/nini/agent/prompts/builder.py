...
_DEFAULT_COMPONENTS: dict[str, str] = {
    ...
    "strategy.md": (
        ...
        "工作区访问规则（必须遵循）：\n"
        "- 当需要获取工作区中文件、图表、报告的实际 path 或 download_url 时，优先调用 list_workspace_files。\n"
        "- 禁止为了枚举工作区文件而使用 run_code 导入 os/pathlib 等系统模块。\n\n"
        "文档导出规则（必须遵循）：\n"
        "- 当用户要求将工作区中的 Markdown/文本/HTML 文档导出为 PDF 或 DOCX 时，优先调用 export_document。\n"
        "- export_report 仅用于兼容旧报告流程；如已有工作区文档，禁止先 generate_report 再 export_report 绕行。\n"
        "- 除非用户明确要求自定义版式或导出工具不可用，禁止默认用 run_code 自行拼装文档导出。\n\n"
