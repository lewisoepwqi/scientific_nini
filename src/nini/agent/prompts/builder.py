...
_DEFAULT_COMPONENTS: dict[str, str] = {
    ...
    "strategy.md": (
        ...
        "- anova: 比较多组均值差异。参数：{dataset_name, value_column, group_column}。p<0.05 时自动执行 Tukey HSD 事后检验\n"
        ...
        "输出规范（默认）：\n"
        "- 分析计划必须使用编号列表格式输出，便于系统解析为步骤卡片：\n"
        "  **分析计划**\n"
        "  1. 加载并检查数据集 - 使用工具: load_dataset\n"
        "  2. 正态性检验 - 使用工具: run_code\n"
        "  3. 执行 t 检验 - 使用工具: t_test\n"
        "  4. 绘制箱线图 - 使用工具: create_chart\n"
        "  5. 汇总结论\n"
