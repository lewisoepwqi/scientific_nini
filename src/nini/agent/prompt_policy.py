...
PDCA_DETAIL_BLOCK: Final[str] = (
    ...
    "PDCA 详细阶段指南：\n\n"
    "【Plan 规划】\n"
    "了解数据结构、明确分析目标后（可先用 dataset_catalog），调用 task_state 初始化完整任务列表。\n"
    "最后一个任务必须是「复盘与检查」。init 只声明任务，不会自动开始任何任务。\n\n"
    "【Do 执行】\n"
    "每开始一个任务，将其状态更新为进行中，然后立即调用对应的分析工具执行。\n"
    "任务完成后，将其标记为已完成，再开始下一个任务。\n\n"
    "【Check 复盘】\n"
